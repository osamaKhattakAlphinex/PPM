...
export default async function PricingPage({
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-16 sm:py-20">
        ...
        {PLANS.map((plan) => (
          <section
            ...
            className={cn(
              ...
              plan.featured ? "border-accent shadow-md" : "border-border",
            )}
          >
            {plan.featured && (
            ...
            <p className="mt-5 font-display text-3xl font-semibold tabular-nums numeric-isolate text-foreground">
              {plan.monthly === null
                ? t("custom")
                : format.number(plan.monthly, { style: "currency", currency: "SAR" })}
              {plan.monthly !== null && (
            ...
            <Link href={localeHref("/contact", locale)} className="mt-7">
      ...
      <p className="mt-10 max-w-2xl text-sm text-muted-foreground">{t("note")}</p>
    </div>
  );
}
